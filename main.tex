...
See following for a sample program that implements measurement. 
\begin{verbatim}
...
QServer also has the `read' instruction, which lifts a bit from the server internal state back into the interpreter environment
to be used in further circuit generation.
When the server receives a read instruction,
it must pause simulation and hold the current simulation state in memory. Here is a
sample program in Python that implements the read command. 
