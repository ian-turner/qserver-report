...
to simulate dynamic lifting, since Qiskit does not allow communication between the Python runtime and the
circuit execution runtime during simulation.
...
the variable \texttt{result}. We keep the \texttt{updated\_state} for executing the future QServer
command. The above program can also be visualized by
...
See following for a sample program that implements measurement of qubit $0$ on statevector
`\verb|phi|'.
\begin{verbatim}
