...
For this benchmark, we implement several quantum adders:
the `QFT adder' \cite{ruiz2017quantum}, the `Gidney adder' \cite{gidney2018halving} (which
is a hybrid quantum-classical algorithm, requiring dynamic lifting), and the `Cuccaro adder'
\cite{cuccaro2004new}.
The Cirq and Qiskit simulators perform significantly worse than the Haskell simulator for these circuits,
as shown in Figure \ref{fig:adder_results}.
This is likely due to how the Haskell simulator stores quantum states.
The Haskell simulator stores the statevector as a map from bit strings to amplitudes.
