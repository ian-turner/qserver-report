...
\begin{abstract}
    ...
    lack native support for \textit{dynamic lifting}. Dynamic lifting is a feature in Quipper
    ...
    We then optimize our new simulators using instruction queuing.
    Finally, we show that our simulators outperform the existing
    simulator on a variety of benchmarks.
...
and Proto-Quipper \cite{fu2023proto}, they provide advanced features such as automatic uncomputation, type-checking, dynamic
...
The measurement results from the circuit execution runtime can then be used 
in the circuit generation runtime to inform the generation of the next circuit.
Thus dynamic lifting allows the two runtimes to be interleaved seamlessly.

While dynamic lifting enables programmers to create much more complex circuits than
...
In particular, we build an interpreter that can run Proto-Quipper programs that use dynamic lifting
...
a simulator. 
The client evaluates Proto-Quipper programs and produces a sequence of low-level quantum
...
includes a read command that reads the results of the measurements.
...
the state-of-the art simulators, especially programs with dynamic lifting. 
As it turns out, the client-server architecture is flexible
...
the simulators' capabilities to enable running Proto-Quipper
programs that use dynamic lifting.
Second, we optimize our simulators by queuing instructions
which do not need to be evaluated immediately.
Finally, we show that our new simulators significantly outperform
...
all initialized to the $\ket{0}$ state, then apply a series of in-place operations on those qubits and bits, and then (usually)
measure the entire qubit array at the end and return the final qubit and bit states.
...
where operations can be conditioned on previous measurement outcomes, which is called \textit{dynamic circuits}. 
However, these features are still experimental and quite limited, and by themselves would be insufficient
to simulate dynamic lifting, since they still do not allow communication between the Python runtime and the
...
In order to integrate Cirq and Qiskit with Proto-Quipper, we must interpret each QServer command
to a quantum circuit object in Cirq and Qiskit.
...
between the two very simple. The only thing we need to handle outside of OpenQASM is the `read'
command from QServer, which we will discuss in more details in Section \ref{sub:sim-qiskit}. 
See Figure \ref{fig:full_sim_diagram} for a diagram of how we extend the
Proto-Quipper's server-client architecture with OpenQASM based simulators.  
...
\label{sub:sim-qiskit}

The QServer language contains four types of quantum operations: qubit initialization, unitary gates, qubit measurement
and read command. We will now discuss how we implement these operations in Qiskit. The implementation will work
similarly for Cirq. 

\paragraph{Qubit Initialization}
Qubits can be initialized in either the $|0\rangle$ or the $|1\rangle$ states.
...
The process is the same for initializing a qubit in the $|1\rangle$ state, except an $X$-gate is applied
to the initialized qubit, and the resulting state is $|1\rangle \otimes |\psi\rangle$.
For example, we can use the following Qiskit program to simulate QServer's qubit initialization in
state $|1\rangle$ with existing state $|\psi\rangle$. 
\begin{verbatim}
qc = QuantumCircuit(4)
...
\end{verbatim}
Here \texttt{phi} is an previous existing three qubit state. We call the \texttt{sim.run} method
to use Qiskit simulator to run the quantum circuit \texttt{qc} and store the simulation result in
the variable \texttt{result}. We keep the \texttt{updated\_state} for executing future QServer
commands. The above program can also be visualized by
...
Unitary gates can be implemented in a similar manner to initialization. See the
following program for an example.
\begin{verbatim}
qc = QuantumCircuit(3)
...
\end{verbatim}
The current simulation state \texttt{phi} is passed in using Qiskit's \texttt{initialize} function,
and the unitary gate \texttt{h} is applied to the qubit 0.
The updated state is then extracted from the simulation results
as a statevector,
and stored in the variable \texttt{phi\_updated} for use in future computations.
See the following circuit visualization.
...
Measurements can be implemented directly on the statevector without creating circuit objects.
...
See following for a sample program pseudo-code that implements measurement of qubit $0$ on statevector
`\verb|phi|'.
\begin{verbatim}
b, phi_updated = measure(phi, [0])
phi.remove([0])
\end{verbatim}

\paragraph{Read command}

QServer also has the `read' instruction, which lifts a bit from the server internal
state back into the interpreter environment to be used in further circuit generation.
When the server receives a read instruction,
it must pause simulation while it sends the result to the client and waits for further instruction.
The server does this by executing the non-measurement instructions which have
accumulated in the queue, then performing measurement as described in the previous paragraph,
and saving the collapsed statevector
in memory to be re-initialized when the next section of the circuit needs to be executed.
The following pseudo-code roughly shows how this is accomplished in Python.
Here, the `measure' function of the simulator uses the Qiskit measurement function and
saves the collapsed state vector as internal state (if there are any remaining qubits after measurement),
and `register' is an integer corresponding to the QServer register to be measured.
Returning a reply simply sends data over the network socket to the client.
\begin{verbatim}
simulator.execute_queue()
result = simulator.measure(register)
return Reply(result)
\end{verbatim}
...
For example, consider the simulation of the following circuit.
...
\]
If not queued, our server will generate the following Qiskit program.
...
run through the simulator. However, if using the queuing technique,
our server will execute the following more efficient program.
...
\end{verbatim}
Here the queued circuit with initialization of a statevector
$|\psi\rangle$ on qubits 3 and 4, initialization of qubits 1 and 2 in
the $|0\rangle$ state, and several queued unitary gates, resulting in
an updated state $|\psi'\rangle$.



\subsection{Converting to OpenQASM}

We use OpenQASM as an intermediate representation between the QServer language and the quantum simulators.
QServer commands are parsed as they are received over the socket connection with the client, and, if the simulator is
...
For example, consider the following Proto-Quipper program for Bell measurement.
...
bellMeas =
  let (a, b) = bell00 ()
  in (dynlift (Meas a), dynlift (Meas b))
\end{verbatim}

When evaluating \texttt{bellMeas}, the Proto-Quipper interpreter will generate the
following QServer commands:
\[
\texttt{Q 0; H 0; Q 1; CNOT 1 0; M 0; R 0; M 1; R 1}
\]
Here \texttt{Q}, \texttt{M} and \texttt{R} are the initialization, measurement and the
read commands respectively. When receiving the measurement commands, the our Qiskit simulation server will generate the following OpenQASM code.
...
In order to evaluate the simulators we construct a series of benchmark programs in Proto-Quipper.
...
and the MaxCut QAOA algorithm.
We compare the queued and non-queued (normal) versions of the updated server against the original Haskell-based server.
To further assess the performance of the server-client simulation architecture,
we also compile the benchmark circuits which do not include dynamic lifting
to OpenQASM and run them directly on the Cirq and Qiskit simulators using Python.
We show that our simulator outperforms the Haskell simulator on most of our benchmark circuits,
except those involving very low degrees of entanglement and superposition.
We also find that the overhead added by the Proto-Quipper client-server interaction is minimal
compared to the overall simulation time.
As a result, we can now scale simulation to almost 30 qubits, where the Haskell simulator
usually crashes at around 16-20.

\paragraph{Quantum Fourier Transform}
The results for QFT (Figure \ref{fig:qft}) show that the queued
versions of the Cirq and Qiskit simulator significantly outperform the
original Haskell simulator as the number of qubits increases.  The
Haskell simulator crashes when trying to simulate more than 20 qubits,
while the Qiskit and Cirq simulators are able to simulate 28 qubits.
They also show that while there is some overhead associated with
the client-server simulation architecture, this
overhead is almost constant as the circuit increases in size, and the
simulation time of the server almost converges to the simulation time
of the bare simulator at around 25 qubits.
...
For this benchmark, we implement several quantum adders: the `QFT
adder' \cite{ruiz2017quantum}, the `Gidney adder'
\cite{gidney2018halving} (which is a hybrid quantum-classical
algorithm, requiring dynamic lifting), and the `Cuccaro adder'
\cite{cuccaro2004new}.  The Cirq and Qiskit simulators perform
significantly worse than the Haskell simulator for the Gidney adder and Cuccaro adder, as
shown in Figure \ref{fig:adder_results}(b)(c).  This is likely due to how
the Haskell simulator stores quantum states.  The Haskell simulator
stores the statevector as a map from bit strings to amplitudes.  For
example, the state $(|00\rangle + |11\rangle)/\sqrt{2}$ is stored as
$[(00, 1/\sqrt{2}), (11, 1/\sqrt{2})]$.  This means that if the
statevector is not in a superposition, then the Haskell representation
needs only one complex number to describe it.  This is in contrast to
the Cirq and Qiskit simulators, which store the entire statevector,
with $2^n$ entries (for $n$ qubits).  As a result, the Haskell
simulator will almost always outperform the Cirq and Qiskit simulators
on circuits that involve mostly classical operations, such as adders.
Of course, useful quantum algorithms will inevitably contain many
non-classical operations, resulting in a large degree of superposition
and entanglement, so in practice this limitation is largely
irrelevant.
...
The maxcut quantum approximate optimization algorithm \cite{farhi2014quantum}
finds the partition of a graph into two graphs such that the
...
Silq \cite{bichsel2020silq}, another high-level language, runs on its own custom simulator,
...
This work presents an approach for simulating Proto-Quipper programs on state-of-the-art third-party
quantum simulators.
This approach extends the number of qubits able to be simulated compared to the existing Proto-Quipper simulator,
but is still significantly limited by memory constraints.
...
Future work could also include compiling Proto-Quipper source code directly to a low-level intermediate format, such as QIR or OpenQASM.
